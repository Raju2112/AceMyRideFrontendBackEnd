										...
										<div class="col-sm-6">
											...
											<div class="form-group">
												<label for="imagePath">Upload Product Image</label> <input
													...
													aria-describedby="fileHelp" name="file"> <small
				...
				<c:forEach items="${listProducts}" var="product">
					<div class="product-card col-sm-8 col-sm-offset-2"
						...
						<div class="card-block row">
							<div class="col-sm-3">
								<img
									src="<c:url value="download?fileName=${product.imagePath}"/>"
									class="product-image">
							</div>
							<div class="col-sm-9">
								<h4 class="card-title" data-name="${product.name }">${product.name }</h4>
								<h6 class="card-subtitle mb-2 text-muted"
									data-productId="${product.productId }">${product.productId }</h6>
								<p>
									Brand: <span class="font-weight-bold"
										data-brand="${product.brand }">${product.brand }</span>
								</p>
								<p>
									Price: <span class="font-weight-bold"
										data-price="${product.price }">${product.price }</span> &nbsp;
									Quantity: <span class="font-weight-bold"
										data-quantity="${product.inStock }">${product.inStock }</span>
								</p>
								<p>
									Category: <span class="font-weight-bold"
										data-category="${product.belongsToCategoryName}">${product.belongsToCategoryName}</span>
									&nbsp; Supplier: <span class="font-weight-bold"
										data-supplier="${product.suppliedBySupplierName }">${product.suppliedBySupplierName }</span>
								</p>
								<p class="description">
									Description: <span class="text-muted"
										data-description="${product.description }">${product.description }</span>
								</p>
								<span class="edit-product btn btn-success"
									data-target="#updateProduct" data-toggle="modal">Edit</span>
							</div>
						</div>
					</div>
				</c:forEach>
			...
			.on(
					...
					function(event) {
						$(this).find('input[name=id]').val(
								$(event.relatedTarget).parents('.product-card')
										.attr('data-id'));
						var sibs = $(event.relatedTarget).siblings();
						$(this).find('input[name=name]').val(
								$(sibs[0]).attr('data-name'));
						$(this).find('input[name=productId]').val(
								$(sibs[1]).attr('data-productId'));
						$(this).find('input[name=brand]').val(
								$(sibs[2]).find('span[data-brand]').attr(
										'data-brand'));
						$(this).find('input[name=price]').val(
								$(sibs[3]).find('span[data-price]').attr(
										'data-price'));
						$(this).find('input[name=inStock]').val(
								$(sibs[3]).find('span[data-quantity]').attr(
										'data-quantity'));
						$(this).find('select[name="belongsToCategoryName"]')
								.val(
										$(sibs[4]).find('span[data-category]')
												.attr('data-category'));
						$(this).find('select[name="suppliedBySupplierName"]')
								.val(
										$(sibs[4]).find('span[data-supplier]')
												.attr('data-supplier'));
						$(this).find('textarea[name=description]').val(
								$(sibs[5]).find('span[data-description]').attr(
										'data-description'));
					});
